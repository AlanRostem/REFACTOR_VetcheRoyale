...
const TileCollider = {
        ...
        toCell(x, y) {
            return {
                x: x / TileCollider.TILE_SIZE | 0,
                y: y / TileCollider.TILE_SIZE | 0,
            };
        },

        toPos(cx, cy) {
            return {
                x: cx * TileCollider.TILE_SIZE,
                y: cy * TileCollider.TILE_SIZE,
            };
        },
                            ...
                            "X":
                                (entity, tile, deltaTime) => {
                                    if (entity.overlapTile(tile)) {
                                        ...
                                        if (entity.vel.x < 0) {
                                            if (entity.pos.x < tile.x + TileCollider.TILE_SIZE) {
                            ...
                            "Y":
                                (entity, tile, deltaTime) => {
                                    if (entity.overlapTile(tile)) {
                                        ...
                                        if (entity.vel.y < 0) {
                                            if (entity.pos.y < tile.y + TileCollider.TILE_SIZE) {
